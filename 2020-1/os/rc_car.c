...
int main()
{
		...
		if((msgq=msgget((key_t)1234,IPC_CREAT | 0666))==-1){
				perror( "msgget() Fail");
				return(-1);
		}
		clrscr();
...
void Controller(int msgq)
{
		...
		while(1){
				msg.key=getch();
				if (-1 == msgsnd(msgq, &msg, sizeof(msg) - sizeof(long), msg.data_type))
				{
						perror( "msgsnd() 실패");
						exit(1);
				}
...
void MoveCar(int msgq)
{
		...
		while(repeat){
				...
				Message m;
				m.data_type=0;
				if(-1!=msgrcv(msgq, &m, sizeof(m) - sizeof(long),m.data_type,IPC_NOWAIT)){
						key = m.key;
						ret = m.data_type;
				}
