...
void MatrixMul_MT(Matrix *mat1, Matrix *mat2, Matrix *res, int no_thread)
{
		...
		pthread_t pid[no_thread];

		//Thread init
		for(int i=0;i<no_thread;i++){
				param[i].no_thread=no_thread;
				param[i].thread_number=i;
				param[i].mat1=mat1;
				param[i].mat2=mat2;
				param[i].res=res;
		}
		...
		//thread create
		for(int i=0;i<no_thread;i++){
				if(pthread_create(&pid[i],NULL,MatrixMul_Thread,(void*)&param[i])<0){
...
void* MatrixMul_Thread(void *vparam)
{
		...
		for(int i = param->thread_number; i < param->mat1->row; i+=param->no_thread){
				for(int j = param->thread_number; j < param->mat2->col; j+=param->no_thread){
						float sum = 0.F;
						for(int k = param->thread_number; k < param->mat1->col; k+=param->no_thread)
								sum += param->mat1->data[i * param->mat1->col + k] * param->mat2->data[k * param->mat2->col + j];
						param->res->data[i * param->res->col + j] = sum;
				}
		}
		return NULL;
}
