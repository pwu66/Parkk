$(function() {
  ...
  $(document).ready(function() {
    $("#submit").click(function() { //검색란에 제출을 클릭
      ...
      //*********** rakuten ***********
      if (r_fselectValue == 0) { //초기값이 0인데, 그대로 0인 경우는 selectbox 선택 하지 않는 경우임
        var temp = ["0"]; // 아무것도 선택 안했을 때는 전체에서 검색. 전체는 장르값을 0으로 설정 해둠.
        post('http://ubuntu@54.199.177.237:5000/rakuten_searched', submit_para, $("#input").val(), temp, 1); //1은 라쿠텐 표시 변수
      } else if (r_sselectValue == 0) { //대분류만 선택
        Bcate_check_fuc();
        //post('http://ubuntu@54.199.177.237:5000/rakuten_searched', submit_para, $("#input").val(), genreId2, 1);
      } else if (r_tselectValue == 0) { // 중분류 선택
        post('http://ubuntu@54.199.177.237:5000/rakuten_searched', submit_para, $("#input").val(), genreId2, 1);
      } else { //소분류까지 선택한 경우
        var temp = [String(r_thirdCategoryArray[r_tselectValue - 1].genreId)]; //소분류까지 선택한 경우
      ...
      //  temp.push(String(r_thirdCategoryArray[r_tselectValue - 1].genreId));
        console.log(temp);
        post('http://ubuntu@54.199.177.237:5000/yahoo_searched', submit_para, $("#input").val(), temp, 3);
      }
  ...
  // 두번째 셀렉트 박스 만들기/  두번째 셀렉트 박스 만들기/  두번째 셀렉트 박스 만들기/  두번째 셀렉트 박스 만들기/  두번째 셀렉트 박스 만들기/  두번째 셀렉트 박스 만들기/
  function r_cate_ajax() {
    // genreId = new Array();
    // genreId2 = new Array();
    ...
    $.ajax({
      ...
      success: function(result) {
        result.filter(function(element) { // children안에 하위 카테고리 id가 들어가 있는데, 그 id의 genreid를 넘겨줘야, 하위카테고리 정보를 얻을 수 있다.
          for (var j = 0; j < r_mainCategoryArray[r_fselectValue - 1].children.length; j++) {
            if (element.id == r_mainCategoryArray[r_fselectValue - 1].children[j]) {
               // g_json = new Object();
               // g_json.genreId =r_mainCategoryArray[r_fselectValue-1].genreId;
               // genreId2.push(element.genreId);
            ...
            //  console.log(r_secCategoryArray);
              r_subCategorySelectBox.append("<option value='" + element.sub_category_id + "'>" + element.cateName + "</option>");
            }
          }
          //   if(genreId.children!=0){ //중분류까 선택한 경우
          //   for(i=0;i<Object.keys(genreId).length;i++){
          //     for(j=0;j<Object.keys(genreId[i]).children.length;j++){
        ...
        // console.log(r_mainCategoryArray[r_fselectValue-1]);
        //   genreId2.push(String(r_mainCategoryArray[r_fselectValue-1].genreId));
        //   console.log(genreId2);

      //    genreId2.push(String(g_json.genreId));
//js-review-widget
      ...
      success: function(result) {
        result.filter(function(element) {
          for (var j = 0; j < r_secCategoryArray[r_sselectValue - 1].children.length; j++) {
            ...
            //   g_json = new Object();
              // g_json.genreId = r_secCategoryArray[r_sselectValue-1].genreId;
              genreId2.push(String(element.genreId));
              element.main_category_id = String(r_fselectValue);
  ...
  function a_First_cate_click() {
    ...
    if (submit_check) { //검색한 경우
      ...
      //  item_ajax('http://ubuntu@54.199.177.237:5000/amazon_selected_ranking','','',2);
  ...
  function y_makingSecondSelectBox() {
    ...
    $.ajax({
      ...
      success: function(result) {
        ...
        console.log(y_mainCategoryArray[y_fselectValue - 1].genreId);
        y_genreId.push(String(g_json.genreId));
      }
    });
